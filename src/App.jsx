...
function App() {
  ...
  const [view, setView] = useState('overview'); // overview, frequent, spend, records
  ...
  const NavButton = ({ id, label, icon: Icon }) => (
    <button 
      ...
      onClick={() => { setView(id); setSelectedMerchant(null); }}
    >
      <Icon size={18} />
      <span>{label}</span>
    </button>
  ...
  const MerchantCard = ({ m, rank }) => (
    <div className="glass merchant-card-big" onClick={() => setSelectedMerchant(m.name)}>
      <div className="merchant-rank-badge">{rank}</div>
      <div className="merchant-info">
        <h3>{m.name}</h3>
        <div className="merchant-meta">
          <span>{m.count} Payments</span>
          <span className="dot"></span>
          <span>{formatCurrency(m.amount)} Total</span>
        </div>
      </div>
    ...
    <div className="container">
      <header>
        ...
        <div className="nav-group">
          ...
          <NavButton id="frequent" label="Most Frequent" icon={Repeat} />
          <NavButton id="spend" label="Highest Spend" icon={TrendingUp} />
          <NavButton id="records" label="Transactions" icon={History} />
        </div>
        <button className="btn-sync" onClick={() => setShowSyncModal(true)} title="Sync with AI">
          ...
          <div className="glass modal-content">
            ...
            <div className="sync-data-box">
              ...
              <button className="copy-btn" onClick={() => { navigator.clipboard.writeText(localStorage.getItem('txn_overrides')); setCopied(true); setTimeout(()=>setCopied(false),2000); }}>
                {copied ? <Check size={16}/> : <Copy size={16}/>} Copy for AI
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="view-fade-in">
          {view === 'overview' && (
            <>
              ...
              <div className="glass glass-card mb-4">
                <h2 className="mb-4">Net Worth Trend</h2>
                <div className="chart-container">
                  <ResponsiveContainer width="100%" height="100%">
          ...
          {view === 'frequent' && (
            <div className="merchant-list">
              <h2 className="mb-4">Most Frequent Payees</h2>
              {merchantStats.frequent.slice(0, 20).map((m, i) => <MerchantCard key={m.name} m={m} rank={i+1} />)}
            </div>
          )}

          {view === 'spend' && (
            <div className="merchant-list">
              <h2 className="mb-4">Highest Expenditure</h2>
              {merchantStats.spending.slice(0, 20).map((m, i) => <MerchantCard key={m.name} m={m} rank={i+1} />)}
            </div>
          )}
        ...
        <div className="modal-overlay">
          <div className="glass modal-content">
            ...
            <div className="flex gap-2 mt-4">
              <button className="btn-save" onClick={()=>handleSaveEdit(editingId)}>Save Changes</button>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .btn-sync { background: var(--surface-color); border: 1px solid var(--border-color); color: var(--text-secondary); padding: 8px; border-radius: 12px; cursor: pointer; }
        .btn-sync:hover { color: var(--accent-color); border-color: var(--accent-color); }
